...
\paragraph{Classical KGE (Milestone~1--2).} We implement \textsc{TransE} and \textsc{RotatE}~\citep{sun2019rotate} with PyKEEN for a fair \textbf{first} line: same splits, $d$, optimiser, training budget, and \textbf{filtered} evaluation. The scoring function is learned; training uses margin/ranking loss and negative sampling.

\paragraph{Custom negative mining.} In parallel with KGE, we will implement a \textbf{learnt or scheduled hard-negative} mechanism: e.g.\ a small $g_\phi$ over a subset of corruptions, or periodic \textbf{mining} in embedding space, with ablations \textbf{against} uniform and Bernoulli \textbf{defaults.}

\paragraph{Optional: relational GNN.} If time allows, a shallow \textsc{R-GCN} (or \textsc{CompGCN}) in PyG/DGL~\citep{schlichtkrull2018modeling} on the \textbf{same} task and splits; not required for the main thesis (hard negatives $+$ KGE $+$ analysis).
...
\paragraph{Preliminary (Milestone~1).} We use \texttt{code/train\_baseline\_kge.py} (or the Colab notebook). For Milestone~2 we target full baseline runs: $d=128$, batch 512, up to 50 epochs, early stopping with patience 10 on validation MRR (\texttt{--full-baseline} or \texttt{LONG\_RUN=True} in the notebook). PyKEEN reports \textbf{realistic} filtered ranking, head and tail prediction averaged.

\paragraph{Test protocol.} Hyperparameters and stopping rely on \textbf{validation} MRR only; \textbf{test} metrics are reported once for the frozen baseline configuration used as reference before hard-negative experiments.

\begin{table}[h]
\centering
\small
\caption{Preliminary filtered metrics (realistic \texttt{both}). Short budget: 15 epochs max, bs=1024, patience~3. Replace with full baseline numbers (50 ep cap, bs=512, patience~10) once Colab/local artifacts are merged.}
\label{tab:baseline_prelim_kg}
...
RotatE consistently outperforms TransE on this preliminary budget: MRR improves from 0.128 to 0.234 (+83\%), and H@10 from 0.264 to 0.378. Both models were still improving at epoch 15 (no early stopping triggered), so longer full-baseline runs should replace Table~\ref{tab:baseline_prelim_kg} for conclusive comparisons. The head vs.\ tail gap (RotatE H@10: 0.261 vs.\ 0.494) motivates relation-type slice analysis.
