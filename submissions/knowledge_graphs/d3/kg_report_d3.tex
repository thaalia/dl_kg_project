...
Our central result is a \emph{controlled within-pipeline ablation}:
across five runs that share identical splits, seed, loss, optimiser, and
Stage~1 sampler, \textbf{hard-negative selection raises MRR from 0.270 to
0.298 (+10.4\%), with the best mixed variant (70\% hard) reaching 0.299};
a 30\% hard mixture already recovers 95\% of that gain. The PyKEEN
TransE/RotatE baselines are reported as external context, since their
internal sampler and training loop differ from our pipeline.
...
\begin{itemize}
  ...
  \item \textbf{(Central result)} A \emph{within-pipeline controlled
    ablation} over five Stage~2 selection strategies (random, hard,
    mixed-30/50/70), with all other factors fixed (same splits, seed,
    loss, optimiser, and Stage~1 sampler), isolating the causal effect of
    the selection rule.
  \item Relation-frequency and entity-degree \textbf{slice evaluation}
    computed entirely from the training graph (no leakage), pinpointing
    where the hard-negative gain concentrates.
  \item A structured \textbf{qualitative analysis} on five representative
    test triples, comparing the ranking of the gold entity across all five
    strategies.
  \item \textsc{TransE} and \textsc{RotatE} baselines via PyKEEN, provided
    as \emph{external reference points} alongside the controlled custom
    ablation (and explicitly not as the central scientific claim, since
    PyKEEN's internal training loop differs from our pipeline).
\end{itemize}
...
\subsection{Comparison with PyKEEN baselines (external context)}
\label{sec:pykeen-context}

The custom ablation in Table~\ref{tab:ablation} is our \emph{central
controlled result}: all five runs share identical splits, seeds, loss,
optimiser, and Stage~1 infrastructure, so the only varying factor is the
Stage~2 selection rule. The comparison below is provided only as external
context against a standard library implementation, and is \emph{not}
...
(MRR~$=0.270$) actually falls slightly below the baselines, which suggests
that the gain comes from hard-negative \emph{selection} and not from the
two-stage infrastructure itself. We stress, however, that the PyKEEN runs
use PyKEEN's internal sampler and training loop, which differ from our
pipeline before Stage~2, so any cross-pipeline comparison should be read as
indicative rather than conclusive (see Limitation~3 in
