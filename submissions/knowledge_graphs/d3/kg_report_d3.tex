...
a 30\% hard mixture recovers 95\% of that gain.
...
Qualitative examples confirm that hard-negative training pushes the correct
entity from mid-list ranks to the top~10 on several structured relation types.
\end{abstract}
...
encodes a binary fact. \textbf{Link prediction} (KB completion) asks: given a
...
strategies~\cite{sun2019rotate} balance exploration and exploitation.

%% ────────────────────────────── 3. DATASET ──────────────────────────────────
\section{Dataset}
\label{sec:dataset}

\textbf{FB15k-237}~\cite{sun2019rotate} is derived from Freebase by removing
...
across strategies---only the final selection differs. All Stage~2 variants thus
add the same $\sim\!20\text{--}30\%$ wall-time overhead compared to standard
sampling.
...
in Deliverable~2 (TransE 0.152, RotatE 0.261), which used simple Bernoulli
sampling with $k=1$ negative per positive and a margin-ranking loss.
...
\midrule
\emph{$\Delta$ (hard vs. random)} & \emph{+0.028} & \emph{+0.028} & \emph{+0.027} & \emph{+0.031} \\
\bottomrule
\end{tabular}
\end{table}
...
infrastructure before Stage2 (see Limitation3 in Section~\ref{sec:discussion}).
...
Tertile cuts computed from training triples only. $n$ = test triples per bucket.}
\label{tab:slice-rel}
\begin{tabular}{@{}lcccc@{}}
\toprule
Strategy & \multicolumn{3}{c}{Relation frequency} \\
\cmidrule(lr){2-4}
         & Rare ($n=849$) & Medium ($n=2172$) & Frequent ($n=17417$) \\
\midrule
Random   & 0.382 & 0.332 & 0.257 \\
Hard     & \textbf{0.418} & \textbf{0.362} & \textbf{0.284} \\
Mixed 30\% & 0.418 & 0.362 & 0.283 \\
Mixed 50\% & 0.414 & 0.363 & 0.285 \\
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Strategy & \multicolumn{3}{c}{Tail in-degree} \\
\cmidrule(lr){2-4}
         & Low ($n=1478$) & Mid ($n=3029$) & High ($n=15849$) \\
\midrule
Random   & 0.157 & 0.154 & 0.301 \\
Hard     & 0.187 & \textbf{0.175} & \textbf{0.330} \\
...
frequent relations (+2.7~pp) are proportionally smaller but still consistent.
...
Gains are more uniform across head-degree buckets ($+2.2$--$+3.2$~pp).
...
\begin{table}[h]
\centering\scriptsize
\caption{Filtered tail rank of the gold entity for two representative test
triples, across all five strategies. Lower rank is better.}
\label{tab:qualitative}
\begin{tabular}{@{}lcc@{}}
\toprule
Strategy & \texttt{/film/film/directed\_by} & \texttt{/music/group\_membership/role} \\
\midrule
Random   & 34  & 47  \\
Hard     &  6  &  8  \\
Mixed 30\% &  7  &  9  \\
...
\begin{itemize}
  \item For \textbf{structured, compositional relations}
    (e.g.\ \texttt{/music/group\_member\-ship/role},
    \texttt{/film/film/directed\_by}),
    hard-negative training pushes the correct entity from rank~20--50
    under random sampling into the top~5 or top~10. This confirms that
    hard negatives specifically help the model learn the fine-grained
    boundaries of compositional relation types.
  \item For \textbf{high-fanout relations}
    (e.g.\ \texttt{/film/film/language}),
    all strategies struggle similarly: the gold entity lands in rank~50--200
    because hundreds of languages plausibly satisfy the query.
    This is a fundamental ambiguity, not a sampling artefact.
  \item \textbf{Mixed strategies} (all three $p$ values) behave nearly
    identically to pure hard on individual examples, consistent with the
    aggregate results showing $<\!0.2$~pp MRR difference.
\end{itemize}

The full qualitative report (5 triples $\times$ 5 strategies, top-10
...
The two-stage pipeline adds $\sim\!20$--$30\%$ wall-time per epoch vs.\
standard Bernoulli sampling, because Stage~2 requires scoring a pool of 64
candidates per positive with the current model.
All five custom runs completed 50 full epochs; total cost was approximately
4--5~hours on a single NVIDIA~T4 GPU.
Baseline training (TransE + RotatE via PyKEEN) added $\sim\!80$ minutes.
Slice evaluation (\texttt{evaluate\_slices.py --all}) required a further
$\sim\!15$--20~minutes.

\paragraph{Synergy with NSSALoss.}
NSSALoss already performs implicit hard-negative up-weighting via the
adversarial weight $p_i$. Explicit hard selection concentrates training on
the same negatives that receive the largest $p_i$ weight, producing a doubly
amplified gradient signal. This synergy likely explains why the gain from
hard selection ($+10.4\%$) is larger than what is typically reported when
hard selection is used with a simpler margin loss.
