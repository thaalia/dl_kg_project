...
%% ───────────────────────────────── ABSTRACT ──────────────────────────────────
\begin{abstract}
We study link prediction on \textbf{FB15k-237} using TransE and
RotatE as matched baselines, then investigate whether the choice of
...
Across five controlled RotatE runs with all other hyper-parameters
fixed (pool size $n=64$, same loss, sampler, splits, seed),
...
We restrict the causal claim to this controlled setting; transfer to other
KGE models or larger candidate pools is left for future work.
...
\subsection{Baseline results (reference, not central claim)}

The two PyKEEN baselines below provide a sanity check against published
KGE results and a reference point for absolute performance levels;
the \emph{central controlled claim} of this report is the custom strategy
ablation in Section~\ref{sec:results-custom} (Table~\ref{tab:custom}),
where all factors except the Stage-2 selection rule are held fixed.

\begin{table}[ht]
\centering\small
\caption{Filtered test-set metrics for the two PyKEEN baselines
(both = head/tail average). Training: NSSALoss ($\gamma=9$, $\alpha=1$),
Bernoulli filtered sampler, $k=8$, 50 epochs, patience 10, $d=128$,
Adam lr=$10^{-3}$, seed~42. Reported as a reference point only;
the controlled comparison is Table~\ref{tab:custom}.}
\label{tab:baselines}
...
\subsection{Custom pipeline: strategy ablation (central controlled result)}
\label{sec:results-custom}

Table~\ref{tab:custom} is the \textbf{central controlled experiment} of this
report. The five rows differ \emph{only} in the Stage-2 selection rule;
model architecture, dimension, loss, optimizer, learning rate, batch size,
candidate-pool size, $k$, sampler, epoch budget, and random seed are all
held fixed. Any difference in test-set metrics is therefore attributable to
the negative-selection strategy.

\begin{table}[ht]
\centering\small
\caption{Filtered test-set metrics for all five custom RotatE runs ---
\textbf{the central controlled comparison}. All runs: pool $n=64$, $k=8$,
NSSALoss ($\gamma=9$, $\alpha=1$), Bernoulli filtered sampler, 50 epochs,
$d=128$, Adam lr=$10^{-3}$, seed~42 (CUDA, T4 GPU). Only the Stage-2
selection rule varies. Best per column in \textbf{bold}.}
\label{tab:custom}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\subsection{Context: position relative to PyKEEN baselines}

For \emph{context only} (not as a controlled comparison), the best custom run
(mixed~70\%, MRR~$=0.299$) sits $+0.020$--$+0.022$ MRR above both PyKEEN
baselines (Table~\ref{tab:baselines}), and the random custom run
(MRR~$=0.270$) sits slightly \emph{below} them. The latter is the more
informative observation: it indicates that the two-stage infrastructure on
its own does not give a free MRR gain, so the improvement seen across
Table~\ref{tab:custom} comes from hard-negative \emph{selection} rather than
from the pipeline scaffolding. We deliberately do \emph{not} treat this as a
causal claim against PyKEEN, because the two pipelines differ in their
internal sampling infrastructure before Stage~2
(see Limitation~3 in Section~\ref{sec:discussion}).
...
This 2$\times$ asymmetry reflects the prevalence of N-to-1 relations:
...
\paragraph{Limitations and scope of conclusions.}
The conclusions of this report are explicitly scoped to the controlled
setting of Table~\ref{tab:custom}. Three caveats follow.
\textbf{(1)~RotatE-only.} All five ablation runs use RotatE with
NSSALoss; we do \emph{not} claim that the same magnitude of gain transfers
to TransE, ComplEx, or other KGE models. NSSALoss already up-weights
plausible negatives, so part of the observed gain may stem from the
synergy between selection-level hard mining and loss-level adversarial
weighting. Replicating the ablation under, e.g., a margin-ranking loss or
a TransE backbone is left for future work.
\textbf{(2)~Fixed pool size $n=64$.} The candidate pool size is held
constant across all five runs. With a smaller pool, hard selection has
fewer candidates to choose from and the effect may shrink; with a larger
pool, the top-$k$ candidates are likely harder and the gap to random may
widen. We therefore present the $+0.028$ MRR figure as evidence under
$n=64$, not as a pool-size-independent constant.
\textbf{(3)~PyKEEN vs.\ custom comparability.} Baseline RotatE via PyKEEN
and custom RotatE via our pipeline differ in their internal sampling
infrastructure before Stage~2; the cross-pipeline numbers in
Section~\ref{sec:results-custom} are reported for context, not as a
controlled comparison. The only fully controlled comparison in this report
is the strategy ablation within Table~\ref{tab:custom}.
...
\begin{enumerate}
  ...
  \item \textbf{Head prediction remains systematically harder} than tail
    ...
    a structural property of N-to-1 relations in FB15k-237 that no
...
\begin{itemize}
  \item \textbf{Thalia Delhaise} implemented the three core pipeline modules
    (\texttt{negative\_sampling.py}, \texttt{score\_candidates.py},
    \texttt{select\_candidates.py}) and their unit-test scripts; designed the
    two-stage pool-then-select architecture.
  \item \textbf{Lenny Briclet} built the baseline and custom training scripts
    (\texttt{train\_baseline\_kge.py}, \texttt{train\_rotate\_custom.py});
    implemented slice and qualitative evaluation
    (\texttt{evaluate\_slices.py}, \texttt{qualitative\_examples.py}); and
    prepared the Colab reproduction notebook.
  \item \textbf{Andr\'e Fonseca} integrated the custom PyTorch training loop
    with NSSALoss and RotatE \texttt{post\_parameter\_update}; ran and
    validated the five-strategy ablation on GPU.
  \item \textbf{Rafael Gufler} led report writing and figure preparation;
    coordinated experiment configuration and artifact management across runs.
\end{itemize}
