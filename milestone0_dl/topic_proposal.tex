...
\title{\textbf{FB15k-237 Link Prediction:\\Beyond Default KGE---Hard Negatives, GNNs, and Relation-Level Analysis}}
\author{%
  \begin{tabular}{rl}
...
PyKEEN gives a fair \textbf{baseline pipeline}; our \textbf{own implementation} (PyTorch / PyG or DGL) covers the \textbf{custom hard-negative} module, optional \textbf{bootstrap} code for CIs, \textbf{slice} evaluation scripts, and the \textbf{GNN} trainer---so the project is not ``only two API calls.''

\textbf{Additional experiments (DL depth).}
\textbf{(1)~Custom negative sampling.} Learned scorer $g_\phi$ and/or periodic hard-negative mining from current embeddings; \textbf{ablate} against uniform/Bernoulli corruption. \textbf{(2)~Slice + uncertainty.} Relation/degree slices; optional bootstrap CIs on the test query set. \textbf{(3)~GNN track.} Shallow \textsc{R-GCN} or \textsc{CompGCN} link predictor on the \textbf{same} splits (independent of~(1)). \textbf{Final readout:} default KGE vs.\ KGE+custom negatives vs.\ GNN, plus relation-wise tables and a short error discussion.
